...
public class UplinkWorker extends Thread {
	...
	public void run() {
		try {
			...
			while (true) {
				...
				if (readLen > 0) {
					...
					if (readLen >= Definition.UPLINK_FINISH_MSG.length() && 
						recvData.substring(readLen - Definition.UPLINK_FINISH_MSG.length(), readLen).equals(Definition.UPLINK_FINISH_MSG)) {
			...
			e.printStackTrace();
			System.out.println("Uplink worker failed: port <" + Definition.PORT_DOWNLINK + ">");
		}
	}
